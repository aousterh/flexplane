...
(1) get the source code.
    git clone git@github.com:aousterh/flexplane.git
...
(5) setup huge pages, etc. for DPDK
    in src/arbiter, run ./dpdk_setup.sh
(6) set arbiter to use stress test
    in src/arbiter, set IS_STRESS_TEST in control.h to 1
(7) build and run arbiter
    in src/arbiter, make clean && make -j5
    ./run.sh


when done:
     in src/arbiter, run ./dpdk_teardown.sh
